...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (flwData.status !== 'success' || flwData.data.status !== 'successful' || flwData.data.amount < plan.price) {
            await client.end();
            return res.status(400).json({ error: 'Payment Failed or Insufficient Amount' });
        }
